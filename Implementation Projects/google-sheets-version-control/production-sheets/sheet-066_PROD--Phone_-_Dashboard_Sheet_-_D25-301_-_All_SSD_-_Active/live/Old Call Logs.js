...
function fetchOldCDRSmain() {
  ...
  const Date_ = new Date(2023,8,29).getTime()
  ...
  for (var i = 0; i < resultObj.cdrs.length; i++) {
    ...
    let createdAtDate = new Date(resultObj.cdrs[i].created_at).getTime()
  ...
  let todaysSheet = ss.getSheetByName("Old_Call_Logs");
  let todaysData = []
  if (todaysSheet.getLastRow() > 1) {
    todaysData = todaysSheet.getRange(2, 1, todaysSheet.getLastRow() - 1, 16).getValues();
  }
