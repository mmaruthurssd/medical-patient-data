...
function makeDevCopiesFromColB() {
  ...
  const COL_D = 4; // original ID
  const COL_F = 6; // new copy hyperlink
  const COL_H = 8; // new copy ID

  const lastRow = sh.getLastRow();
  if (lastRow <= HEADER_ROW) return;

  // Read needed columns in one shot for speed
  const range = sh.getRange(HEADER_ROW + 1, 1, lastRow - HEADER_ROW, 8); // A:H
  ...
  for (let i = 0; i < values.length; i++) {
    ...
    // Write original ID to column D
    sh.getRange(rowIndex, COL_D).setValue(originalId);
    ...
    try {
      ...
      // Write new copy ID to column H
      sh.getRange(rowIndex, COL_H).setValue(newId);
