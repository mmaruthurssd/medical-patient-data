...
function downloadInfoMailAttachments() {
  ...
  const SearchQuery = 'to:"info@ssdspc.com" has:attachment after:' + date;
  ...
  const FolderId = '1uv2zjcwVrxOYInppnES_F2stB11TufoJ';
  const DestFolder = DriveApp.getFolderById(FolderId);

  const DocsLogSheet = SpreadsheetApp.getActive().getSheetByName('Info_Attachments');
  const MsgIdLogSheet = SpreadsheetApp.getActive().getSheetByName('Info_MsgId_Log');
