...
function downloadAdminMailAttachments() {
  ...
  // const SearchQuery = 'label:unprocessed label:unread has:attachment ';
  // const SearchQuery = 'in:inbox has:attachment to:"admin@ssdspc.com" after:' + date;

  const SearchQuery = 'to:"admin@ssdspc.com" has:attachment after:' + date;
  const Threads = GmailApp.search(SearchQuery, 0, 100);



  // const SearchQuery = 'from:stacy has:attachment after:2024/8/6 before:2024/8/7 ';
  //  const SearchQuery = 'subject:gone for good  has:attachment after:2024/11/2 before:2024/12/12 ';
  //  const Threads = GmailApp.search(SearchQuery, 0, 500);

  if (Threads.length === 0) return;

  const FolderId = '1_7_K4Roh7lZjZ4ShvcB4-au_bAukU8xO';
  const DestFolder = DriveApp.getFolderById(FolderId);

  const DocsLogSheet = SpreadsheetApp.getActive().getSheetByName('Admin_Attachments');
  const MsgIdLogSheet = SpreadsheetApp.getActive().getSheetByName('Admin_MsgId_Log');
  ...
  while (index < Threads.length && isTimeLimitReached === false) {
    ...
    Messages.forEach(Message => {
      ...
      if (MsgIdLog.indexOf(MsgId) === -1) {
        ...
        if (!Sender.includes("copier@ssdspc.com")) {
