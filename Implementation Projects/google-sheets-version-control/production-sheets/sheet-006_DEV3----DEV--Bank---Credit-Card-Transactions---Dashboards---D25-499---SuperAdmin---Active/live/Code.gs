...
const OUTPUT_FOLDER_ID = "1uk5RA7Fu-H26sL96B_UUYX_OQz81AAMJ"
...
function onOpen() {
  ...
  menu.addItem("Create Bank Sheet", "createBankSheetAndFolder").addToUi()
}
...
function createBankSheetAndFolder() {
  ...
  const sheet = ss.getSheetByName("Yearly Sheets");
  ...
  for (var i = 0; i < allData.length; i++) {
    ...
    if ((allData[i][5] == true || allData[i][5] == "TRUE") && allData[i][3] == "" && allData[i][4] == "") {
      ...
      if (id == "") {
        id = createNewIdNEW_(sheet, "BCC", i + 1)
      }

      let newOutputFolder = outputFolder.createFolder(allData[i][0] + " - Folder - " + id)
      let newSS = DriveApp.getFileById(SHEET_TEMLATE_ID).makeCopy(allData[i][0] + " - Sheet - " + id, newOutputFolder)

      let folderRich = SpreadsheetApp.newRichTextValue().setText(allData[i][0] + " - Folder - " + id).setLinkUrl(newOutputFolder.getUrl()).build()
      let ssRich = SpreadsheetApp.newRichTextValue().setText(allData[i][0] + " - Sheet - " + id).setLinkUrl(newSS.getUrl()).build()
