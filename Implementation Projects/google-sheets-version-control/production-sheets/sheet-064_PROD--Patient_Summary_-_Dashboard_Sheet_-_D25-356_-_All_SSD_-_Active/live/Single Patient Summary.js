...
function createIndividualPatientSummaryViaMRN_(PatientMrn) {

  const ss = SpreadsheetApp.getActiveSpreadsheet();

  const LogSheet = ss.getSheetByName('Individual_Log');

  if (!PatientMrn || PatientMrn == "") {
    console.log(`Invalid!\nPlease input a valid MRN: ${PatientMrn}`);
    return;
  }

  PatientMrn = PatientMrn.toString().toUpperCase();

  const TodayDate = Utilities.formatDate(new Date(), 'GMT-6', 'M/d/yyyy');
  ...
  const AllIdsObj = {
    ...
    // 'DocsFolder': '1O-bRK-qviTpJTu9qAkVncnw_CRZ5Kg97',
    // 'TempFolder': '1KdXfA1W6QQytjsvjOi3i3tu6c4wXJvsn'
  ...
  //console.log('creating bulk patient summary for ' + ProviderName + ' dated ' + TodayDate);
  ...
  // const PatientApptsData = AllApptsData.filter(row => row[1] === TodayDate && row[2] === ProviderName &&
  // (row[6] === 'Confirmed' || row[6] === 'Pending'));



  //const PatientApptsData = AllApptsData.filter(row => row[0] === PatientMrn &&
  //(row[6] === 'Confirmed' || row[6] === 'Pending'));

  const PatientApptsData = AllApptsData.filter(row => row[0].toString().toLowerCase() === PatientMrn.toString().toLowerCase())

  if (PatientApptsData.length === 0) {
    console.log(`Invalid!\nNo data found for the MRN: ${PatientMrn}`);
    try {
  ...
  const ApptPatientIds = PatientApptsData.map(row => row[0].toString());
  ...
  PatientApptsData.forEach(DataRow => {
    ...
    patientName = DataRow[4]

    PatientsDataObj[PatientId] = [[PatientFullName, PatientDob].join('_'),
    [PatientFirstName, PatientDob].join('_'),
      ApptType];
  ...
  const PastAppts = AllApptsData.filter(row => row[0].toString() === PatientId &&
    new Date(row[1]).getTime() < new Date(TodayDate).getTime());

  const FutureAppts = AllApptsData.filter(row => row[0].toString() === PatientId &&
    new Date(row[1]).getTime() > new Date(TodayDate).getTime());

  const SearchKey = PatientsDataObj[PatientId][1];
  const PatientBiopsies = PreEMAPlusSkinDx.filter(row => [row[7].split(' ')[0], row[8]].join('_') === SearchKey);
  ...
  const [NewNoteDocId, NewNotePdfId] = fillPatientNoteNew_(NewDoc, DocsFolder, DocDataObject,
    PastAppts, FutureAppts, PatientBiopsies,
    UntreatedMalignancies, HighRiskSkinCancers);

  //AllNewDocIds.push(NewNotePdfId);
  const newNotePdf = DriveApp.getFileById(NewNotePdfId)
  newNotePdf.setName("Individual_" + patientName + '_' + PatientMrn + "_" + TodayDate)
  const DataValues = [new Date(), patientName, PatientMrn, newNotePdf.getUrl()];
  LogSheet.appendRow(DataValues);
  //LogSheet.getRange(5, 1, LogSheet.getLastRow() - 4, LogSheet.getLastColumn()).sort([{ column: 1, ascending: false }])
...
function createBulkSinglePatientSummary() {

  const ss = SpreadsheetApp.getActiveSpreadsheet();


  const LogSheet = ss.getSheetByName('Individual_Log');
  const PatientMrnSheet = ss.getSheetByName('Individual Patient Summary');
  const PatientMrn = PatientMrnSheet.getRange("B2").getDisplayValue().toString().toUpperCase();
  ...
  if (PatientApptsData.length === 0) {
    console.log('No data found for the selected MRN ...');
    try {
      const ui = SpreadsheetApp.getUi();
      ui.alert("Invalid!\nNo data found for the selected MRN.");
    } catch (err) {
      console.log(err.toString());
    }
    return
