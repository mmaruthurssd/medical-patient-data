...
function onEdit(e) {
  ...
  if (ActiveSheet.getName() == DAILY_TASK_TEMPLATE) {
    ...
    if (row > 2 && col == 6 && e.value != "" && e.value != null) {
      ...
      if (existingId == "") {
        ...
        ActiveSheet.getRange(row, 2, 1, 2).setValues([["RT-" + newID, "Remote Team"]])
  ...
  if (ActiveSheet.getName() !== 'Microtasks List' && ActiveSheet.getName() !== "Kevin's To Dos" && ActiveSheet.getName() !== "Rodrigo's To Dos" && ActiveSheet.getName() !== "Rashid's To Dos" && ActiveSheet.getName() !== "Alvaro's To Dos" && ActiveSheet.getName() !== "Dr. M's To Dos" )
    return;
