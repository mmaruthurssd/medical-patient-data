...
/**
 * Extract ALL Dev3 Sheets - Including those without Script IDs
 *
 * Column F: Sheet Name
...
async function extractDev3Sheets() {
  try {
    console.log('🔍 Extracting ALL dev3 sheets (script ID optional)...\n');
    ...
    console.log(`📋 Column G: Spreadsheet ID (REQUIRED)`);
    console.log(`📋 Column H: Apps Script Project ID (OPTIONAL)\n`);
    ...
    rows.forEach((row, index) => {
      ...
      // CORRECTED: F=Name, G=Spreadsheet ID, H=Script ID
      const sheetName = row[0];
      const spreadsheetId = row[1];  // Column G (REQUIRED)
      const scriptId = row[2];        // Column H (OPTIONAL)
    ...
    // Generate CSV file
    const csvPath = path.join(__dirname, '../dev3-sheets-ALL.csv');
    ...
    const bashScriptPath = path.join(__dirname, '../add-dev3-ALL-sheets.sh');
    const bashScript = [
      '#!/bin/bash',
      '',
      ...
      'cd "$(dirname "$0")"',
      '',
      'echo "Adding ALL dev3 sheets to registry..."',
      'echo ""',
      '',
      ...registryCommands,
      '',
    ...
    // Print summary
    console.log('═══════════════════════════════════════════════════════');
    console.log('📊 EXTRACTION COMPLETE (ALL SHEETS)');
    console.log('═══════════════════════════════════════════════════════');
    ...
    console.log(`CSV file: dev3-sheets-ALL.csv`);
    console.log(`Bash script: add-dev3-ALL-sheets.sh`);
    console.log('');
    ...
    console.log('  1. Clear old registry (backup first!)');
    console.log('  2. node scripts/bulk-import.js dev3-sheets-ALL.csv');
    console.log('  3. npm run snapshot (will skip sheets without script IDs)');
    console.log('═══════════════════════════════════════════════════════');
    console.log('');
