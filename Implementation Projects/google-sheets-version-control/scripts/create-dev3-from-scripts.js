#!/usr/bin/env node
/**
 * Create DEV3 sheets by:
 * 1. Creating new blank spreadsheets
 * 2. Copying the PROD Apps Script code
 * 3. Binding the script to the new spreadsheet
 *
 * This approach doesn't require access to PROD spreadsheets - only the scripts
 */
...
// Use token.json OAuth credentials
...
const LOG_FILE = path.join(__dirname, '../logs/create-dev3-from-scripts-' + new Date().toISOString().replace(/[:.]/g, '-') + '.log');
...
async function createBlankSpreadsheet(sheetName) {
  log(`  Creating blank spreadsheet: ${sheetName}`);
  ...
  try {
    ...
    log(`  ✗ Failed to create spreadsheet: ${error.message}`);
    throw error;
  }
}

// Copy Apps Script by cloning project
async function copyAndBindScript(sourceScriptId, spreadsheetId, sheetName) {
  log(`  Copying Apps Script and binding to spreadsheet...`);
  ...
  try {
    ...
    // Create a new container-bound script attached to the spreadsheet
    const createResponse = await script.projects.create({
      requestBody: {
        title: `[DEV] ${sheetName}`,
        parentId: spreadsheetId  // Bind to spreadsheet
      }
    });

    const newScriptId = createResponse.data.scriptId;
    log(`  ✓ Created bound script: ${newScriptId}`);
...
async function createLocalDirectory(serial, sheetName, spreadsheetId, scriptId, registryId) {
  ...
  try {
    await execPromise('clasp pull', {
      cwd: path.join(dirPath, 'live')
    });

    log(`  ✓ Code pulled successfully`);

    // Remove .clasp.json from live
    ...
    const files = fs.readdirSync(path.join(dirPath, 'live')).filter(f => !f.startsWith('.'));
...
async function processSheet(sheet) {
  ...
  try {
    ...
    // Create blank spreadsheet
    const newSpreadsheetId = await createBlankSpreadsheet(sheetName);

    // Wait for Drive to process
    await new Promise(resolve => setTimeout(resolve, 2000));

    // Copy and bind script
    const newScriptId = await copyAndBindScript(prodScriptId, newSpreadsheetId, sheetName);
...
async function main() {
  log('========================================');
  log('Create DEV3 Sheets from Scripts');
  log('========================================');
