...
**System:** Google Sheets Version Control (470 Production Sheets)
...
This document defines a **6-layer defense-in-depth backup strategy** to protect 470 production Apps Script projects. Each layer provides redundancy and different recovery scenarios.
...
[alias]
    ...
    verify-sheets = "!f() { count=$(ls -d production-sheets/sheet-* 2>/dev/null | wc -l); echo \"Production sheets: $count (expected: 470)\"; if [ $count -ne 470 ]; then echo 'WARNING: Sheet count mismatch!'; exit 1; fi }; f"
...
echo "✓ Production sheets: $PROD_COUNT (expected: 470)"
